...
corpus = Corpus(
    ...
    min_word_freq=3,
    batch_size=64,
    wv_file=f".{folder_backend}/pretrained/id_ft.bin"
)
...
trainer = Trainer(
    ...
    device=use_device,
    checkpoint_path=f".{folder_backend}/models/{model_name}.pt"
)

trainer.model = NERModel(**configs[model_name]).to(use_device)
trainer.model.load_state(f".{folder_backend}/models/{model_name}.pt")

"""
==============================================================
"""

# # get data from database medmon
...
# def predictAll(df=df):
#     for row_news in enumerate(df):
